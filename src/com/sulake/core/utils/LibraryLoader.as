package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1976:uint = 1;
      
      protected static const const_1977:uint = 2;
      
      protected static const const_1975:uint = 4;
      
      protected static const const_1978:uint = 3;
      ...
      public static const const_1289:Boolean = false;
      
      public static const const_1824:int = 2;
      
      private static var var_1035:Array = new Array();
      ...
      protected var var_411:int = 0;
      
      protected var var_248:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_640:Class;
      
      protected var var_492:uint = 0;
      ...
      protected var var_2969:String = "";
      
      protected var var_2970:String = "";
      
      protected var var_2192:int;
      
      protected var var_1376:int;
      
      protected var var_1375:int;
      
      protected var var_3029:uint = 0;
      
      protected var var_2968:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         if(const_1289)
         {
            if(var_1035.indexOf(param1) == -1)
            {
               var_1035.push(param1);
            }
            throttle();
         }
         else
         {
            param1.var_42.load(param1.var_248,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1289)
         {
            ...
            while(var_823.length < const_1824 && false)
            {
               _loc1_ = var_1035.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_823.push(_loc1_);
                  _loc1_.var_42.load(_loc1_.var_248,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_248 ? this.var_248.url : null;
      }
      ...
      {
         return !!this.var_1107 ? uint(this.var_1376 - this.var_2192) : uint(getTimer() - this.var_2192);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_411,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_42.close();
            }
      ...
      {
         this.var_248 = param1;
         this._name = parseNameFromUrl(this.var_248.url);
         this.var_1107 = false;
         this.var_1375 = param2;
      ...
      {
         if(this.var_1108 && !_disposed)
         {
            this.var_1108 = false;
            if(!this.var_1107 && this.var_248)
            {
               this.var_1376 = -1;
      ...
      {
         if(!this.var_1107 && !_disposed && this.var_1375 > 0)
         {
            try
            {
               this.var_42.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_248,const_1821 - this.var_1375);
            if(this.var_2968)
            {
               this.var_248.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_248.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2968 = false;
            }
            --this.var_1375;
            this.var_492 = 0;
            this.var_42.load(this.var_248,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_411 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_411 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_411,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_411,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_411))
               {
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_492 = LibraryLoader.name_4;
         ...
         if(this.var_492 == LibraryLoader.const_1978)
         {
         ...
         if(this.var_492 == LibraryLoader.name_4)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_411,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_640 = this.getDefinition(this._name) as Class;
         if(this.var_640 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_248.url + "!");
            }
            return false;
         }
         ...
         try
         {
            xmlClass = this.var_640.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_411,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_411,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
