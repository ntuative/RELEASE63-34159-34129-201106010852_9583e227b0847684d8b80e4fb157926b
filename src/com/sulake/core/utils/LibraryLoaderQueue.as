package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1824:int = 4;
      ...
      {
         return this.var_425.length + this.var_292.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_292.length < const_1824 && this.var_425.length > 0)
            {
               _loc1_ = this.var_425.shift();
               this.var_292.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_425.indexOf(loader);
            if(index > -1)
            {
               this.var_425.splice(index,1);
            }
            index = this.var_292.indexOf(loader);
            if(index > -1)
            {
               this.var_292.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_425)
            {
               if(param2)
               {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_292)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
