package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_901:int = 3;
      
      private var var_420:AssetLoaderStruct;
      ...
      private const const_2245:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_420 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_902 = new Timer(this.const_2245,1);
         this.var_902.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_420)
         {
            this.var_420.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_420.addEventListener(AssetLoaderEvent.const_40,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_420)
         {
            this.var_420.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_420.removeEventListener(AssetLoaderEvent.const_40,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_901 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1118);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_901);
            if(this.var_1118.indexOf("?") > 0)
            {
               _loc2_ = this.var_1118 + "&retry=" + this.var_901;
            }
            else
            {
               _loc2_ = this.var_1118 + "?retry=" + this.var_901;
            ...
            this.var_420 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2197,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1117[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1117[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1652)
         {
            this.var_1116.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_563[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_563[_loc3_] = _loc6_;
            }
            this.var_903[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_903)
         {
            _loc7_ = true;
            _loc4_ = this.var_903[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_563[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_903[_loc3_];
         }
         while(_loc6_ < this.var_720.length)
         {
            _loc5_ = this.var_720[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_720.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_902.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_721.length > 0 && this.var_720.length < this.const_1824)
         {
            _loc1_ = this.var_721[0];
            _loc1_.startDownloading();
            this.var_720.push(this.var_721.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_721.indexOf(param1) == -1 && this.var_720.indexOf(param1) == -1)
         {
