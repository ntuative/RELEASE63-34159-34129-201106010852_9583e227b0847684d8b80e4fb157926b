package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_71:IAvatarEditorCategoryModel;
      
      private var _window:IWindowContainer;
      
      private var var_1602:IWindow;
      
      private var var_875:IFigurePartSet;
      ...
      private var var_701:Rectangle;
      
      private var var_2119:int = 0;
      
      private var var_479:IAvatarRenderManager;
      
      private var var_1110:Boolean;
      
      private var var_2118:BitmapData;
      
      private const const_2287:int = 13421772;
      
      private const const_2288:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_707 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_2119 = Math.max(this.var_2119,_loc6_.colorLayerIndex);
            }
         }
         this.var_479 = this.var_71.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_707 != null && !this.var_2117)
            {
               _loc3_ = this.var_707;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_875)
            {
               switch(this.var_875.clubLevel)
               {
                  case HabboClubLevelEnum.const_55:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_37:
                     _loc2_.style = HabboIconType.const_708;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_741;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1602.color = this.const_2287;
         }
         else
         {
            this.var_1602.color = this.const_2288;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_479.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_479.isFigureReady(_loc1_))
         {
            this.var_479.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_370[_loc6_] + "_" + FigureData.const_165;
               _loc4_ = this.var_479.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_701 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2118)
               {
                  _loc9_ = this.var_71.controller.assets.getAssetByName("avatar_editor_download_icon");
                  this.var_2118 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_701.width,this.var_701.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_701.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_701.y;
               if(this.var_2117 && _loc2_.colorLayerIndex > 0)
               {
