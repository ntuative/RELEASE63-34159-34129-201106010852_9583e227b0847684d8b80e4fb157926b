package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridView
   {
      ...
      public function AvatarEditorGridView(param1:IAvatarEditorCategoryModel, param2:String, param3:IHabboWindowManager, param4:IAssetLibrary)
      {
         ...
         if(this._view != null)
         {
            this.var_679 = this._view.findChildByName("part_thumbs") as IItemGridWindow;
            this.var_562 = new Array();
            this.var_562.push(this._view.findChildByName("palette0") as IItemGridWindow);
            this.var_562.push(this._view.findChildByName("palette1") as IItemGridWindow);
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(this.var_562)
         {
            for each(_loc1_ in this.var_562)
            {
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
      ...
      public function initFromList() : void
      {
         ...
         var _loc5_:* = null;
         var _loc6_:* = null;
         var _loc1_:CategoryData = this.var_71.getCategoryData(this.var_1586);
         if(!_loc1_)
         {
            return;
         }
         this.var_679.removeGridItems();
         for each(_loc2_ in _loc1_.parts)
         {
            if(_loc2_)
            {
               this.var_679.addGridItem(_loc2_.view);
         ...
         _loc3_ = 0;
         while(_loc3_ < this.const_1622)
         {
            _loc4_ = _loc1_.getPalette(_loc3_);
            _loc5_ = this.var_562[_loc3_] as IItemGridWindow;
            if(!(!_loc4_ || !_loc5_))
            {
      ...
      {
         var _loc3_:IWindow = this.var_679.getGridItemAt(param1);
         if(!_loc3_)
         {
            return;
         }
      ...
      private function partEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            _loc3_ = this.var_679.getGridItemIndex(param1.window);
            this.var_71.selectPart(this.var_1586,_loc3_);
         }
      }
      ...
      private function paletteEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_44)
         {
            ...
            _loc7_ = 0;
            while(_loc7_ < this.const_1622)
            {
               if(this.var_562.length > _loc7_)
               {
                  _loc4_ = this.var_562[_loc7_] as IItemGridWindow;
                  _loc5_ = _loc4_.getGridItemIndex(_loc6_);
                  if(_loc5_ > -1)
                  {
                     this.var_71.selectColor(this.var_1586,_loc5_,_loc7_);
                     return;
                  }
               }
               _loc7_++;
