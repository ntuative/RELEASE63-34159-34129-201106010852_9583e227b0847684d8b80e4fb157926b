package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_113,this.onTextWindowEvent);
         ...
         this.var_515 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_515 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2304 = _loc3_.localizationId;
         this.var_349 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_515,this);
               if(_loc5_)
               {
         ...
         this._offers.add(this.var_515,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_923,this.onApproveNameResult);
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_49)
         {
            ...
            if(this.var_199 == null || _loc3_ >= this.var_199.length)
            {
               return;
            }
            this.var_730 = _loc3_;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_502 = param1.index;
         if(this.var_502 < 0 || this.var_502 > this.var_349.length)
         {
            this.var_502 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2304)
         {
            this.var_199 = this.parseSellableBreeds(param1.sellableBreeds);
            this.var_730 = 0;
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_515 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         {
            _loc5_ = this.var_199[_loc3_] as int;
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_515,_loc5_),this.getRaceLocalizationKey(this.var_515,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_199 == null || this.var_730 >= this.var_199.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_502 >= 0 && this.var_502 < this.var_349.length)
         {
            _loc7_ = this.var_349[this.var_502] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_199[this.var_730] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_515,_loc3_,_loc2_,AvatarScaleType.const_58,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_328,AvatarAction.const_762);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_43,2);
         this.setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_43),true);
         _loc4_.dispose();
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_502 >= this.var_349.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_349[this.var_502] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_199[this.var_730] as int;
      ...
      private function updateAvailableBreeds(param1:String) : void
      {
         ...
         if(this.var_199 == null)
         {
            ...
            this.var_199 = this.parseSellableBreeds(_loc2_);
            this.var_730 = 0;
            if(this.var_199 != null)
            {
               this.updateBreedSelections();
