package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer, param2:ISessionDataManager)
      {
         ...
         this.var_637 = [0,0,0];
         this.var_106 = ["group.walls","group.floors","group.views"];
         super(param1,param2);
         this.var_429 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_429)
         {
            _loc3_ = 0;
            while(_loc3_ < this.var_429.numSelectables)
            {
               _loc4_ = this.var_429.getSelectableAt(_loc3_);
               if(_loc4_ is ISelectableWindow)
               {
                  _loc4_.addEventListener(WindowEvent.const_49,this.onSelectGroup);
               }
               _loc3_++;
            }
         }
      ...
      override public function dispose() : void
      {
         ...
         var _loc2_:* = null;
         super.dispose();
         for each(_loc1_ in this.var_207)
         {
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_106[this.var_527]);
      ...
      {
         var _loc2_:int = this.var_637[this.var_527];
         var _loc3_:Offer = this.var_207[this.var_527][_loc2_];
      ...
      {
         var _loc2_:* = null;
         if(param1 > -1 && param1 < var_484.numGridItems)
         {
            _loc2_ = this.var_207[this.var_527][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_637[this.var_527] = (this.var_207[this.var_527] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_637[0];
         var _loc2_:int = this.var_637[1];
         var _loc3_:int = this.var_637[2];
         var _loc4_:Offer = this.var_207[0][_loc1_];
         var _loc5_:Offer = this.var_207[1][_loc2_];
         var _loc6_:Offer = this.var_207[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_409 || _loc1_.pricingModel == Offer.const_410))
            {
               continue;
            }
            ...
            if(!(_loc2_.productType == ProductTypeEnum.const_66 || _loc2_.productType == ProductTypeEnum.const_79))
            {
               continue;
            }
            ...
            while(this.var_207.length < this._groupNames.length)
            {
               this.var_207.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_207[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_207[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         if(_loc2_ > -1)
         {
            ...
            if(var_484)
            {
               var_484.destroyGridItems();
            }
            page.replaceOffers(this.var_207[this.var_527],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_637[this.var_527];
