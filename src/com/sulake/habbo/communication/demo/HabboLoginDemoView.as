package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_539:String = "InitConnection";
       
      
      private var var_170:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_142:SSOTicket;
      ...
      {
         this.var_170.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_170 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_170.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_165,HabboWindowStyle.const_48,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_170.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_2006,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_170.habboConfiguration)
            {
               if(this.var_170.habboConfiguration.getKey("use.sso","false") == "true")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_170.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_170.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_2006,"/");
         ...
         {
            dispatchEvent(new Event(const_539));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_170.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_142.removeEventListener(SSOTicket.const_429,this.onParsedTicketSuccess);
            this.var_142.removeEventListener(SSOTicket.const_326,this.onParsedTicketFailure);
            this.var_142 = null;
         }
         var _loc2_:String = "";
         if(this.var_170.habboConfiguration != null)
         {
            _loc2_ = this.var_170.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_170.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_142 = new SSOTicket(this.var_170.assets,_loc3_,_loc2_,param1);
         this.var_142.addEventListener(SSOTicket.const_429,this.onParsedTicketSuccess);
         this.var_142.addEventListener(SSOTicket.const_326,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_142.ticket);
         Logger.log("Has Facebook: " + this.var_142.isFacebookUser);
         if(this.var_142.isFacebookUser)
         {
            this.var_170.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_170.flashClientUrl = this.var_142.flashClientUrl;
         this.var_170.ssoTicket = this.var_142.ticket;
         this.var_142.removeEventListener(SSOTicket.const_429,this.onParsedTicketSuccess);
         this.var_142.removeEventListener(SSOTicket.const_326,this.onParsedTicketFailure);
         this.var_142 = null;
         dispatchEvent(new Event(const_539));
