package com.sulake.habbo.friendlist
{
   ...
   public class FriendListTabsView
   {
      ...
      public function refresh(param1:String) : void
      {
         ...
         this.var_178.width = this._friendList.tabs.tabContentWidth;
         this.var_178.findChildByName("bg").width = this._friendList.tabs.tabContentWidth;
         var _loc2_:int = 1;
         for each(_loc3_ in this._friendList.tabs.getTabs())
         {
            _loc4_ = this.var_178.getChildByName("flt_" + _loc3_.id) as IWindowContainer;
         ...
         this.var_178.height = _loc2_ + 1;
         this.var_178.findChildByName("bg").height = this.var_178.height;
         Logger.log("TABS: REFRESH END");
      }
      
      private function isTabVisible(param1:int) : Boolean
      {
         if(param1 != FriendListTab.const_99)
         {
            return true;
         }
      ...
      private function refreshHeader(param1:FriendListTab, param2:IWindowContainer) : int
      {
         ...
         var _loc4_:Boolean = param1.id == FriendListTab.const_111 && !param1.newMessageArrived;
