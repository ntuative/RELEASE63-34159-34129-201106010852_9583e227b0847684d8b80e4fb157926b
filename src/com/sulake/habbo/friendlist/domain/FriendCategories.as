package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_451:IFriendCategoriesDeps;
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_416);
      ...
      public function getFriendCount(param1:Boolean, param2:Boolean = false) : int
      {
         var _loc4_:* = null;
         var _loc3_:int = 0;
         for each(_loc4_ in this.var_106)
         {
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_451.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_451.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_451.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_451.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_451.view.setNewMessageArrived();
                  this.var_451.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_416).received = true;
         this.findCategory(FriendCategory.const_1332).received = true;
         ...
         for each(_loc3_ in this.getCategoriesNotReceived())
         {
            ...
            {
               Util.remove(this.var_106,_loc3_);
               _loc3_.dispose();
            }
         }
      }
