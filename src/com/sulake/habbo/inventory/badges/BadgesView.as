package com.sulake.habbo.inventory.badges
{
   ...
   public class BadgesView implements IInventoryView
   {
      ...
      private var var_71:BadgesModel;
      
      private var var_645:IItemGridWindow;
      
      private var var_1004:IItemGridWindow;
      ...
      public function BadgesView(param1:BadgesModel, param2:IHabboWindowManager, param3:IAssetLibrary)
      {
         ...
         this.var_645 = this._view.findChildByName("inactive_items") as IItemGridWindow;
         this.var_645.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onInactiveBadgeClick);
         this.var_1004 = this._view.findChildByName("active_items") as IItemGridWindow;
         this.var_1004.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onActiveBadgeClick);
      }
      ...
      private function updateListViews() : void
      {
         ...
         var _loc1_:Number = this.var_645.scrollV;
         this.var_645.removeGridItems();
         this.var_1004.removeGridItems();
         var _loc2_:Array = this.var_71.getBadges(BadgesModel.const_464);
         var _loc4_:int = 0;
         while(_loc4_ < _loc2_.length)
         {
            ...
            if(!_loc3_.isInUse)
            {
               this.var_645.addGridItem(_loc3_.window);
         ...
         if(_loc1_ > 0)
         {
            this.var_645.scrollV = _loc1_;
         }
         var _loc5_:Array = this.var_71.getBadges(BadgesModel.const_308);
         _loc4_ = 0;
         while(_loc4_ < _loc5_.length)
         {
            _loc3_ = _loc5_[_loc4_] as Badge;
            this.var_1004.addGridItem(_loc3_.window);
      ...
      public function updateActionView() : void
      {
         ...
         var _loc2_:Badge = this.var_71.getSelectedBadge();
         if(_loc2_ == null)
         {
            _loc1_.disable();
         ...
         else
         {
            ...
            this.setBadgeName(this.var_71.controller.localization.getBadgeName(_loc2_.type));
            this.setBadgeDescriptionText(this.var_71.controller.localization.getBadgeDesc(_loc2_.type));
            this.setBadgeDescriptionImage(_loc2_.iconImage);
            _loc3_ = this.var_71.getBadges(BadgesModel.const_308);
            if(_loc3_ != null && _loc3_.length >= this.var_71.getMaxActiveCount() && !_loc2_.isInUse)
            {
               _loc1_.disable();
            }
            else
      ...
      {
         var _loc2_:int = this.var_645.getGridItemIndex(param1.window);
         var _loc3_:Badge = this.var_71.getBadgeFromInactive(_loc2_);
         if(_loc3_ != null)
         {
            this.var_71.setBadgeSelected(_loc3_.type);
         }
      }
      
      private function onActiveBadgeClick(param1:WindowEvent) : void
      {
         var _loc2_:int = this.var_1004.getGridItemIndex(param1.window);
         var _loc3_:Badge = this.var_71.getBadgeFromActive(_loc2_);
         if(_loc3_ != null)
         {
            this.var_71.setBadgeSelected(_loc3_.type);
         }
      ...
      private function onWearBadgeClick(param1:WindowEvent) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.var_71.toggleBadgeWearing(_loc2_.type);
         }
      }
   }
}
