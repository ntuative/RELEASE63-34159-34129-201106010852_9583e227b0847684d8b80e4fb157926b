package com.sulake.habbo.navigator
{
   ...
   public class PopupCtrl
   {
      ...
      {
         this.var_434 = new Timer(500,1);
         this.var_361 = new Timer(100,1);
         super();
         this._navigator = param1;
         this.var_1111 = param4;
         this.var_2519 = param2;
         this._popupIndentLeft = param3;
         this.var_434.addEventListener(TimerEvent.TIMER,this.onDisplayTimer);
      ...
      public function dispose() : void
      {
         ...
         if(this.var_434)
         {
            this.var_434.removeEventListener(TimerEvent.TIMER,this.onDisplayTimer);
            this.var_434.reset();
            this.var_434 = null;
         }
         if(this.var_361)
      ...
      public function showPopup(param1:IWindow) : void
      {
         if(this.var_55 == null)
         {
            this.var_55 = IWindowContainer(this._navigator.getXmlWindow(this.var_1111));
            this.var_55.visible = false;
            this.var_55.setParamFlag(HabboWindowParam.const_46,true);
            this.var_55.procedure = this.onPopup;
         }
         Util.hideChildren(this.var_55);
         this.refreshContent(this.var_55);
         this.var_55.height = Util.getLowestPoint(this.var_55) + 5;
         var _loc2_:Point = new Point();
         param1.getGlobalPosition(_loc2_);
         this.var_55.x = _loc2_.x + this.var_2519 + param1.width;
         this.var_55.y = _loc2_.y - this.var_55.height * 0.5 + param1.height * 0.5;
         var _loc3_:Point = new Point();
         this.var_55.getGlobalPosition(_loc3_);
         if(_loc3_.x + this.var_55.width > this.var_55.desktop.width)
         {
            this.var_55.x = -this.var_55.width + _loc2_.x + this._popupIndentLeft;
      ...
      private function onPopup(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(!Util.containsMouse(this.var_55))
            {
               this.closePopup();
            }
         }
