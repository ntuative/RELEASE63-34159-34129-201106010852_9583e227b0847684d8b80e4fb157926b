package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_2594:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2594 = new RoomThumbnailRenderer(param1);
         this.var_1048 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_186 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_186.visible = true;
         Util.hideChildren(this.var_186);
         this.refreshRoomName(this.var_186,this.var_118);
         if(this.var_118.ownerName != "" && this.var_118.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_186,!!this.var_118.event ? this.getEventCtg(this.var_118.categoryId) : this.getRoomCtg(this.var_118.categoryId));
         this.refreshRoomDesc(this.var_186,this.var_118);
         this.refreshExtraCont();
         this.refreshInfo(this.var_186,"trading_allowed",this.var_118.allowTrading);
         this.refreshInfo(this.var_186,"doormode_doorbell",this.var_118.doorMode == 1);
         this.refreshInfo(this.var_186,"doormode_password",this.var_118.doorMode == 2);
         Util.moveChildrenToColumn(this.var_186,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_186.height = Util.getLowestPoint(this.var_186);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_186,this.var_118.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_186.findChildByName("roomowner"));
         if(this.var_118.ownerName != "" && this.var_118.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_186.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1048.refreshTags(_loc1_,this.var_118.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_118.score);
         if(this.var_118.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_118.eventCreationTime);
         }
         this.var_2594.refreshThumbnail(_loc1_,this.var_118.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_201,Tabs.const_253,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
