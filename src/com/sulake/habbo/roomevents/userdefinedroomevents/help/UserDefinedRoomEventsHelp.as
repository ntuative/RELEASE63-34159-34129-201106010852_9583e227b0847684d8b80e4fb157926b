package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_86.getXmlWindow("ude_help"));
         this.var_1812 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1182 = this.var_1812.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1182)
         {
            this.var_1182.procedure = this.onButtonClick;
         }
         this.var_1183 = this.var_1812.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1183)
         {
            this.var_1183.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_254 >= this.var_64.length - 1)
         {
            this.var_1183.disable();
         }
         else
         {
            this.var_1183.enable();
         }
         if(this.var_254 <= 0)
         {
            this.var_1182.disable();
         }
         else
         {
            this.var_1182.enable();
         }
      }
      ...
      {
         ++this.var_254;
         this.var_254 = Math.min(this.var_254,this.var_64.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_254;
         this.var_254 = Math.max(this.var_254,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_64 = [];
         var _loc1_:int = 1;
         while(this.var_86.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_86.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_86.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_86.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
            this.var_64.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_64 || this.var_64.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_64[this.var_254][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_64[this.var_254][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_64[this.var_254][1]).value;
      ...
      {
         if(!param1 || !this.var_86 || !this.var_86.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_86.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_86.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_86.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
