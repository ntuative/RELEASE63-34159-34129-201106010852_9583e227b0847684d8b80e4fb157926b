package com.sulake.habbo.session.furniture
{
   ...
   public class FurnitureDataParser extends EventDispatcher
   {
      ...
      public static const const_117:String = "ready";
      ...
      private var var_1511:Map;
      
      private var _localization:ICoreLocalizationManager;
      
      private var var_794:AssetLibrary;
      ...
      public function FurnitureDataParser(param1:Map, param2:Map, param3:Map, param4:ICoreLocalizationManager)
      {
         ...
         this.var_794 = new AssetLibrary("FurniDataParserAssetLib");
      }
      ...
      {
         var _loc2_:AssetLoaderStruct = this.var_794.loadAssetFromFile(param1,new URLRequest(param1),"text/plain");
         _loc2_.addEventListener(AssetLoaderEvent.const_29,this.parseFurnitureData);
         _loc2_.addEventListener(AssetLoaderEvent.const_40,this.furnitureDataError);
      }
      ...
      private function parseFurnitureData(param1:AssetLoaderEvent) : void
      {
         ...
         for each(_loc10_ in _loc8_)
         {
            ...
            for each(_loc12_ in _loc11_)
            {
               ...
               if(_loc14_ == "s")
               {
                  this.var_494.add(_loc15_,_loc31_);
               }
               else if(_loc14_ == "i")
               {
                  this._wallItems.add(_loc15_,_loc31_);
               }
               _loc32_ = this.var_1511[_loc17_];
               if(_loc32_ == null)
               {
                  _loc32_ = [];
                  this.var_1511.add(_loc17_,_loc32_);
               }
               _loc32_[_loc18_] = _loc15_;
            }
            _loc9_++;
         }
         dispatchEvent(new Event(const_117));
      }
      ...
      public function registerFurnitureLocalizations() : void
      {
         ...
         for each(_loc1_ in this.var_494.getValues())
         {
