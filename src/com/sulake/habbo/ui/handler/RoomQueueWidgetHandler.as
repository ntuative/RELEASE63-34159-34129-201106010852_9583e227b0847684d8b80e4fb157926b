package com.sulake.habbo.ui.handler
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRoomQueueMessage.const_721);
         _loc1_.push(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_978);
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_721:
               this._container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               this._container.roomSession.changeQueue(RoomSessionQueueEvent.const_1259);
               break;
            case RoomWidgetRoomQueueMessage.const_978:
               this._container.roomSession.changeQueue(RoomSessionQueueEvent.const_1251);
               break;
            case RoomWidgetRoomQueueMessage.const_816:
               if(this._container.catalog != null)
               {
                  this._container.catalog.openCatalogPage(CatalogPageName.const_178,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_536];
      }
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_536:
               ...
               {
                  case RoomSessionQueueEvent.const_1251:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_1259:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc5_.length > 1)
               {
                  if(_loc4_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_1313) != -1)
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1313) + 1;
                     _loc7_ = true;
                  }
                  else
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1914) + 1;
                  }
               }
               else
               {
