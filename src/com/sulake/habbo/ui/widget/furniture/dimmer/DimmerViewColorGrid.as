package com.sulake.habbo.ui.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_500:IItemGridWindow;
      
      private var _view:DimmerView;
      
      private var var_1701:XML;
      
      private var var_596:BitmapData;
      
      private var var_1137:BitmapData;
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_500.numGridItems)
         {
            return;
         }
         this.select(this.var_500.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_500 == null)
         {
            return;
         }
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         {
            _loc3_ = param1.buildFromXML(this.var_1701) as IWindowContainer;
            ...
            _loc3_.width = this.var_596.width;
            _loc3_.height = this.var_596.height;
            this.var_500.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_596.width,this.var_596.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_596,this.var_596.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1137.width,this.var_1137.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(this.var_916.width,this.var_916.height,true,16777215);
               _loc6_.bitmap.copyPixels(this.var_916,this.var_916.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = this.var_500.getGridItemIndex(param1.target as IWindow);
