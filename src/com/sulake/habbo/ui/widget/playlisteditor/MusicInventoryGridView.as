package com.sulake.habbo.ui.widget.playlisteditor
{
   ...
   public class MusicInventoryGridView
   {
      ...
      private var var_597:IItemGridWindow;
      
      private var _items:Map;
      
      private var _widget:PlayListEditorWidget;
      
      private var var_102:MusicInventoryGridItem;
      ...
      public function destroy() : void
      {
         ...
         {
            this.var_597.destroyGridItems();
            this.var_597 = null;
         }
         if(this.var_225 != null)
         {
            if(this.var_225.events != null)
            {
      ...
      public function refresh() : void
      {
         ...
         this.var_597.removeGridItems();
         var _loc1_:Map = this._items;
         var _loc2_:Map = new Map();
         var _loc3_:Array = _loc1_.getKeys();
         this._items = new Map();
         var _loc4_:int = this.var_225.getSongDiskInventorySize();
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc7_ = this.var_225.getSongDiskInventoryDiskId(_loc5_);
            _loc8_ = this.var_225.getSongDiskInventorySongId(_loc5_);
            _loc9_ = this.var_225.getSongInfo(_loc8_);
            _loc10_ = null;
            _loc11_ = null;
            if(_loc9_ != null)
            {
            ...
            this.var_597.addGridItem(_loc12_.window);
      ...
      public function setPreviewIconToPause() : void
      {
         ...
         {
            this.var_102.playButtonState = MusicInventoryGridItem.const_1184;
         }
      }
      ...
      public function setPreviewIconToPlay() : void
      {
         if(this.var_102 != null)
         {
            this.var_102.playButtonState = MusicInventoryGridItem.const_552;
         }
      }
      ...
      private function gridItemEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         var _loc4_:int = 0;
         var _loc5_:* = null;
         var _loc3_:* = param1.type == WindowMouseEvent.const_159;
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK || _loc3_)
         {
            if(param2.name == "button_to_playlist" || _loc3_)
            {
               if(this.var_102 != null)
               {
                  this.var_102.deselect();
                  this.stopPreview();
                  this._widget.sendAddToPlayListMessage(this.var_102.diskId);
                  this.var_102 = null;
               }
            }
            else if(param2.name == "button_play_pause")
            {
               if(this.var_102.playButtonState == MusicInventoryGridItem.const_552)
               {
                  this.var_102.playButtonState = MusicInventoryGridItem.const_1201;
                  this._widget.playUserSong(this.var_102.songId);
               }
               else
               {
                  this.stopPreview();
            ...
            {
               _loc4_ = this.var_597.getGridItemIndex(param1.window);
               if(_loc4_ != -1)
               {
                  _loc5_ = this._items.getWithIndex(_loc4_);
                  if(_loc5_ != this.var_102)
                  {
                     if(this.var_102 != null)
                     {
                        this.var_102.deselect();
